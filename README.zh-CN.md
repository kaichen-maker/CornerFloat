...
如果当前输入/输出组合可能触发蓝牙双向通话降质，CornerFloat 会在网站的 WebKit
提示之前显示原生说明。有可用的内建输入时，选择 **Use Mac Microphone** 可临时
切换系统默认输入，**Continue with Bluetooth** 会保持当前蓝牙路由不变，
**Cancel** 则拒绝本次请求。没有内建替代设备时只提供后两个选择。只有用户明确
点击后才会发生临时切换。
...
- 网站请求麦克风前，遇到可能降低语音质量的蓝牙路由会先显示原生说明；用户可
  明确选择临时使用 Mac 麦克风、保持蓝牙不变继续，或取消。
...
在有风险的蓝牙输入/输出路由上，CornerFloat 会在 WebKit 网站提示前只读取完成
本机语音质量判断所需的设备信息。有可用的内建输入时会提供
**Use Mac Microphone**、**Continue with Bluetooth** 和 **Cancel**；否则只提供
继续或取消。临时使用 Mac 麦克风会修改系统默认输入，因此会话期间其他音频应用
也可能看到该设备。网站停止捕获、面板关闭或 CornerFloat 退出时，应用会尽力
恢复原输入，但只有当前输入仍是 CornerFloat 临时选择的设备时才恢复；用户期间
或其他应用后来更改默认输入且被系统事件检测到时，CornerFloat 会立即放弃恢复权。
设备选择不会被持久保存，CornerFloat 也不会复制、分析、保存麦克风音频，或把
音频发送到 CornerFloat 服务。

Apple 说明，同一蓝牙耳机同时用于播放与麦克风时，macOS 会切换到音质较低的
双向模式，直到麦克风停止使用。详见
[Apple 蓝牙耳机支持说明](https://support.apple.com/zh-cn/102217)。只有明确选择
Mac 麦克风才会避免这次路由切换；继续使用蓝牙会保留当前系统路由。
...
- 网站语音或听写只允许 HTTPS 页面请求，并且必须分别通过 macOS 和网站层授权；
  ...
  CornerFloat，再重新载入网页。
- 蓝牙风险提示只读取本机路由判断所需的设备信息；临时系统输入切换必须由用户
  明确选择，并且恢复时不会覆盖用户之后手动选择的输入。
