...
[从源码运行](docs/SOURCE_BUILD.md) · [路线图](docs/ROADMAP.md) · [安全政策](SECURITY.md)
...
从 GitHub 源码使用不需要 Apple 开发者账号、Developer ID、公证或 Passkey 特殊授权：
...
`make run` 会在本机编译、进行 ad-hoc 临时签名并打开 `dist/CornerFloat.app`。这种构建适合源码学习、开发和个人使用。只有当维护者要向普通用户分发可直接下载的正式二进制时，才需要 Developer ID、公证和签名更新通道。

运行 `make install` 可把本地构建安装到 `~/Applications`，不需要 `sudo`；`make uninstall` 只移除该应用副本，保留偏好、资料库和 WebKit 网站数据。资料库也可先从 Settings 导出备份。

可选的正式发行版通过 `CornerFloat-<version>-macOS-universal.dmg` 分发：

1. 打开 DMG；
2. 把 `CornerFloat.app` 拖入 **Applications**；
3. 从 Applications 启动 CornerFloat；
4. 完成首次启动引导。

CornerFloat 是菜单栏应用，因此正常运行时不占用 Dock。启动后可从菜单栏的 CornerFloat 图标打开 ChatGPT、其他网页或资料库。

仓库中的本地构建产物位于：

```text
dist/CornerFloat.app
dist/CornerFloat-macOS-<native-arch>.zip
dist/CornerFloat-0.8.0-macOS-universal.dmg
```

本地产物默认只有临时签名，适合开发和个人测试。只有由 Developer ID 签名、Apple 公证并装订公证票据的发行产物，才应作为公开下载版本。
